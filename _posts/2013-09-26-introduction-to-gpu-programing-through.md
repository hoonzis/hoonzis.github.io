...
Before going further with more examples we have to provide basic
...
significantly with the release of any new version. Nevertheless certain
concepts are shared by all of them. I will describe here shortly the
NVidias Fermi and ATIs Cypress architecture. The successor of Fermi is
called
...
GPUs usually run computations on data which is accessed only once, unlike CPUs where the same memory pages are often used repeatedly. For this reason it is better for the developer to control the local memory. That's why historically the local memory did not act as cache on GPUs. However this will probably change in the future, and some form of caching will be provided by the GPU directly.
...
article I will try to describe how AMP addresses these issues.
...
void vectorsAddition_Parallel(vector<float> vA, vector<float> vB, vector<float>vC, int M, int N){
 ...
 //capture of the data using array_view -> results in the copy of the data into the accelerators memory.
 parallel_for_each(e,[=](index<2>idx) restrict(amp){
  c[idx] = a[idx] + b[idx];
 });
}
...
there is an **array** class which behaves similarly, having few
...
The following example illustrates some of the difficulties which we can
...
The algorithm adds each two neighboring items and stores the result in
...
values in an array. First step is the computation of the average of the
array. To obtain the average we have to first add the elements in the
...
Here is one more example which illustrates how index and extent work, it
...
*array* has to be then sent manually back to the CPUs memory. On the
...
to global memory and optimize the threads to use the local memory.
This process is called **tiling** in the AMP's parlance and the local
memory is called **tile-static** memory.
...
be tiled manually. Parallel\_for\_each method has a second overload which
...
### Links
- [Introduction and few samples](http://geekswithblogs.net/JoshReuben/archive/2011/12/04/c-amp.aspx)
- [Parallel Programing in Native Code - blog of the AMP team](http://blogs.msdn.com/b/nativeconcurrency)
- [CUDA architecture evolution](http://www.anandtech.com/show/6846/nvidia-updates-gpu-roadmap-announces-volta-family-for-beyond-2014)
- [GeForce GTX 680 Kepler Architecture](http://www.geforce.com/Active/en_US/en_US/pdf/GeForce-GTX-680-Whitepaper-FINAL.pdf)
- [Comparison between CUDA and OpenCL (though little outdated 2011-06-22)](http://www.streamcomputing.eu/blog/2011-06-22/opencl-vs-cuda-misconceptions/)
- [Introduction to Tiling](http://msdn.microsoft.com/en-us/magazine/hh882447.aspx)
- [Implementation of LLVM & Clang to support AMP C++ on NVidia](https://github.com/corngood)
